...
APP_NAME = os.environ.get('APP_NAME', 'factordopamina')
...
async def login(payload: LoginRequest, response: Response):
    ...
    return {"id": user["id"], "email": user["email"], "name": user.get("name", ""), "role": user.get("role", "admin"), "token": token}
...
async def list_public_courses(featured: Optional[bool] = None):
    ...
    items = await db.courses.find(query, {"_id": 0}).sort("created_at", -1).to_list(200)
    return items

@api_router.get("/courses/{slug}")
async def get_course_by_slug(slug: str):
    course = await db.courses.find_one({"slug": slug, "status": "published"}, {"_id": 0})
    if not course:
...
async def list_all_courses(user: dict = Depends(get_current_user)):
    items = await db.courses.find({}, {"_id": 0}).sort("created_at", -1).to_list(500)
    return items
...
async def download_file(path: str):
    ...
    return FileResponse(content=data, media_type=content_type, headers={"Cache-Control": "public, max-age=86400"})
...
@api_router.get("/")
async def root():
    return {"app": "Factor Dopamina", "status": "ok"}
        ...
        seed_courses = [
            {
                ...
                "modules": [
                    {"title": "Módulo 1 — La Mente del Hombre Disciplinado", "description": "Reprograma tu identidad."},
                    {"title": "Módulo 2 — Rutinas de Alto Rendimiento", "description": "Mañanas, sueño, energía."},
                    {"title": "Módulo 3 — El Sistema de los 1000 Días", "description": "Cómo sostener la disciplina."},
                    {"title": "Módulo 4 — Romper el Ciclo del Confort", "description": "Exposición controlada al desafío."},
                ],
                "status": "published",
                "featured": True,
            ...
            },
            {
                ...
                "modules": [
                    {"title": "Módulo 1 — Anatomía de la Mente Fuerte", "description": "Cómo funciona realmente."},
                    {"title": "Módulo 2 — Diálogo Interno y Narrativa", "description": "Reescribe la voz interna."},
                    {"title": "Módulo 3 — Gestión de Presión", "description": "Operar bajo caos."},
                    {"title": "Módulo 4 — El Hombre Imperturbable", "description": "Estado emocional dominante."},
                ],
                "status": "published",
                "featured": True,
            ...
            },
            {
                ...
                "modules": [
                    {"title": "Módulo 1 — Biología Masculina", "description": "Testosterona, dopamina, cortisol."},
                    {"title": "Módulo 2 — Sueño Profundo", "description": "El multiplicador absoluto."},
                    {"title": "Módulo 3 — Movimiento y Fuerza", "description": "Entrenamiento mínimo eficaz."},
                    {"title": "Módulo 4 — Combustible Real", "description": "Nutrición sin religión."},
                ],
                "status": "published",
                "featured": True,
                "price_label": "Inversión única",
            },
            {
                ...
                "modules": [
                    {"title": "Módulo 1 — La Economía de la Atención", "description": "Por qué pierdes el foco."},
                    {"title": "Módulo 2 — Detox de Dopamina", "description": "Resetear tu sistema."},
                    {"title": "Módulo 3 — Sesiones Deep Work", "description": "Protocolo de 90 minutos."},
                    {"title": "Módulo 4 — Cerebro de Alto Rendimiento", "description": "Hábitos cognitivos."},
                ],
                "status": "published",
                "featured": False,
                "price_label": "Acceso de por vida",
            },
            {
                ...
                "modules": [
                    {"title": "Módulo 1 — La Pregunta del Hombre", "description": "Quién eres realmente."},
                    {"title": "Módulo 2 — Visión a 10 Años", "description": "Diseñar la vida."},
                    {"title": "Módulo 3 — El Mapa de Ejecución", "description": "Trimestres, meses, semanas."},
                    {"title": "Módulo 4 — Vivir con Misión", "description": "Sostener el propósito."},
                ],
                "status": "published",
                "featured": True,
                "price_label": "Inversión única",
